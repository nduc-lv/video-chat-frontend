...
const {Search} = Input;
export default function WatchTogether({roomId, mode, isJoinRoom}:any){
    ...
    return(
        ...
        <div className="flex flex-row h-screen" style={{backgroundColor:"#19202A", width:"100%"}}>
            ...
            <div className="grow" style={{marginTop:"20px", marginLeft: "3vw"}}>
                {videoId == null || <YoutubeVideoPlayer videoId={videoId} roomId={roomId} socket={socket} mode={mode} isJoinRoom={isJoinRoom}></YoutubeVideoPlayer>}
            </div>
        </div>
        </>
    )
